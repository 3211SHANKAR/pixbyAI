...
export default function AvatarGenerator() {
  ...
  const generateAvatar = async () => {
    ...
    setImage(null);
    setTimeout(() => {
      setImage(`https://api.dicebear.com/7.x/adventurer/svg?seed=${encodeURIComponent(prompt || "PixAI")}`);
      setLoading(false);
    }, 1200);
  };
  ...
  return (
    <section className="relative flex flex-col items-center justify-center min-h-[70vh] w-full bg-gradient-to-br from-indigo-50 via-white to-pink-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900 p-0 sm:p-8 overflow-hidden">
      ...
      <Navbar/>
      <div className="relative z-10 flex flex-col items-center w-full max-w-lg bg-white/80 dark:bg-gray-900/80 rounded-2xl shadow-2xl p-8 gap-6 border border-indigo-100 dark:border-gray-800 backdrop-blur-md mt-8">
        <h2 className="text-4xl font-extrabold text-center bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 bg-clip-text text-transparent mb-2 drop-shadow">AI 3D Avatar Generator</h2>
        <p className="text-center text-gray-600 dark:text-gray-300 mb-2">Describe your dream avatar and generate a unique 3D-style image instantly!</p>
